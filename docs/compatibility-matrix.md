...
### Oracle Linux host preparation policy

Oracle Linux can provide vendor preinstallation RPMs for some database
releases, but this repository does **not** rely on them. Use the explicit,
reviewable scripts (`create-oracle-user.sh`, `configure-kernel-params.sh` and
`configure-limits.sh`) so the same workflow remains visible and auditable on
RHEL and compatible distributions. `check-os-prereqs.sh --oracle-version <ver>`
therefore reports base OS packages only; it does not recommend installing an
Oracle preinstallation package.
...
| 11gR2 | RHEL / Oracle Linux 5–6 era | Historical scaffold only; use the repository scripts for explicit user/sysctl/limits preparation. |
...
- Do not rely on Oracle Linux preinstallation RPMs in this repository; keep
  user, limits and sysctl preparation explicit through the documented scripts.
